...
int Error_Test_scatterv(int argc, char **argv)
{
    ...
    switch(test_num)
    {
        case 0:
            ...
            err =  MPI_Scatterv(sendbuf, sendcounts, displs, MPI_INT, recvbuf, size, MPI_INT, root, MPI_COMM_WORLD);
            if(VERBOSE)
            {
                ierr = 0;
        ...
        case 3:
            ...
            if (my_world_rank == 0) 
            {
                err =  MPI_Scatterv(sendbuf, sendcounts, displs, MPI_UNSIGNED, recvbuf, size, MPI_UNSIGNED, root, MPI_COMM_WORLD);
            } 
            else 
            {
                err =  MPI_Scatterv(sendbuf, sendcounts, displs, MPI_INT, recvbuf, size, MPI_INT, root, MPI_COMM_WORLD);
        ...
        case 4:
            ...
            err =  MPI_Scatterv(sendbuf, sendcounts, displs, MPI_INT, recvbuf, size, MPI_INT, root, MPI_COMM_NULL);
        ...
        case 5:
            ...
            if (my_world_rank == 0) 
            {
                err =  MPI_Scatterv(sendbuf, sendcounts, displs, MPI_INT, recvbuf, size, MPI_INT, root, MPI_COMM_SELF);
            } 
            else 
            {
                err =  MPI_Scatterv(sendbuf, sendcounts, displs, MPI_INT, recvbuf, size, MPI_INT, root, MPI_COMM_WORLD);
    ...
    if (err == MPI_SUCCESS) {
        printf("Process %d found no error in scatterv test %d \n", my_world_rank, test_num);
    } else {      
