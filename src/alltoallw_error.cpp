...
int Error_Test_alltoallw(int argc, char **argv)
{
    ...
    switch(test_num)
    {
        ...
        case 5:
            ...
            if (my_world_rank == 0) 
            {
                err = MPI_Alltoallw(sendbuf, sendcounts, sdispls, sendtypes, recvbuf, recvcounts,rdispls, recvtypes, MPI_COMM_SELF);
            } 
            else 
            {
                err = MPI_Alltoallw(sendbuf, sendcounts, sdispls, sendtypes, recvbuf, recvcounts,rdispls, recvtypes, MPI_COMM_WORLD);
    ...
    if (err == MPI_SUCCESS) {
        printf("Process %d found no error in alltoallw test %d \n", my_world_rank, test_num);
    } else {      
