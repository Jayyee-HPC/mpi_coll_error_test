...
int Error_Test_gatherv(int argc, char **argv)
{
    ...
    switch(test_num)
    {
        case 0:
            ...
            err = MPI_Gatherv(sendbuf, size, MPI_INT, recvbuf, recvcounts, displs, MPI_INT, root, MPI_COMM_WORLD);
        ...
        case 3:
            ...
            if (my_world_rank == 0) 
            {
                err = MPI_Gatherv(sendbuf, size, MPI_UNSIGNED, recvbuf, recvcounts, displs, MPI_UNSIGNED, root, MPI_COMM_WORLD);
            } 
            else 
            {
                err = MPI_Gatherv(sendbuf, size, MPI_INT, recvbuf, recvcounts, displs, MPI_INT, root, MPI_COMM_WORLD);
        ...
        case 5:
            ...
            err = MPI_Gatherv(sendbuf, size, MPI_INT, recvbuf, recvcounts, displs, MPI_INT, root, MPI_COMM_NULL);
            break;
        ...
        case 6:
            ...
            if (my_world_rank == root) 
            {
                err = MPI_Gatherv(sendbuf, size, MPI_INT, recvbuf, recvcounts, displs, MPI_INT, root, MPI_COMM_SELF);
            } 
            else 
            {
                err = MPI_Gatherv(sendbuf, size, MPI_INT, recvbuf, recvcounts, displs, MPI_INT, root, MPI_COMM_WORLD);
    ...
    if (err == MPI_SUCCESS) {
        printf("Process %d found no error in gatherv test %d \n", my_world_rank, test_num);
    } else {      
