...
int Error_Test_iallgatherv(int argc, char **argv)
{
    ...
    switch(test_num)
    {
        case 0:
            ...
            err = MPI_Iallgatherv(sendbuf, size, MPI_INT, recvbuf, recvcounts, displs, MPI_INT, MPI_COMM_WORLD, &request);
        ...
        case 3:
            ...
            if (my_world_rank == 0) 
            {
                err = MPI_Iallgatherv(sendbuf, size, MPI_UNSIGNED, recvbuf, recvcounts, displs, MPI_UNSIGNED, MPI_COMM_WORLD, &request);
            } 
            else 
            {
                err = MPI_Iallgatherv(sendbuf, size, MPI_INT, recvbuf, recvcounts, displs, MPI_INT, MPI_COMM_WORLD, &request);
        ...
        case 4:
            ...
            err = MPI_Iallgatherv(sendbuf, size, MPI_INT, recvbuf, recvcounts, displs, MPI_INT, MPI_COMM_NULL, &request);
            MPI_Wait(&request, &status);
        ...
        case 6:
            ...
            if (my_world_rank == 0) 
            {
                err = MPI_Iallgatherv(sendbuf, size, MPI_INT, recvbuf, recvcounts, displs, MPI_INT, MPI_COMM_SELF, &request);
            } 
            else 
            {
                err = MPI_Iallgatherv(sendbuf, size, MPI_INT, recvbuf, recvcounts, displs, MPI_INT, MPI_COMM_WORLD, &request);
    ...
    if (err == MPI_SUCCESS) {
        printf("Process %d found no error in allgatherv test %d \n", my_world_rank, test_num);
    } else {      
