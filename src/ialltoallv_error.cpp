...
int Error_Test_ialltoallv(int argc, char **argv)
{
    ...
    switch(test_num)
    {
        ...
        case 3:
            ...
            if (my_world_rank == 0) 
            {
                err = MPI_Ialltoallv(sendbuf, sendcounts, sdispls, MPI_UNSIGNED, recvbuf, recvcounts,rdispls, MPI_UNSIGNED, MPI_COMM_WORLD, &request);
            } 
            else 
            {
                err = MPI_Ialltoallv(sendbuf, sendcounts, sdispls, MPI_INT, recvbuf, recvcounts,rdispls, MPI_INT, MPI_COMM_WORLD, &request);
        ...
        case 6:
            ...
            if (my_world_rank == 0) 
            {
                err = MPI_Ialltoallv(sendbuf, sendcounts, sdispls, MPI_INT, recvbuf, recvcounts,rdispls, MPI_INT, MPI_COMM_SELF, &request);
            } 
            else 
            {
                err = MPI_Ialltoallv(sendbuf, sendcounts, sdispls, MPI_INT, recvbuf, recvcounts,rdispls, MPI_INT, MPI_COMM_NULL, &request);
    ...
    if (err == MPI_SUCCESS) {
        printf("Process %d found no error in ialltoallv test %d \n", my_world_rank, test_num);
    } else {      
