...
int Error_Test_allgather(int argc, char **argv)
{
    ...
    switch(test_num)
    {
        case 0:
            ...
            err = MPI_Allgather(sendbuf, size, MPI_INT, recvbuf, size, MPI_INT, MPI_COMM_WORLD);
        ...
        case 3:
            ...
            if (my_world_rank == 0) 
            {
                err = MPI_Allgather(sendbuf, size, MPI_UNSIGNED, recvbuf, size, MPI_UNSIGNED, MPI_COMM_WORLD);
            } 
            else 
            {
                err = MPI_Allgather(sendbuf, size, MPI_INT, recvbuf, size, MPI_INT, MPI_COMM_WORLD);
        ...
        case 4:
            ...
            err = MPI_Allgather(sendbuf, size, MPI_INT, recvbuf, size, MPI_INT, MPI_COMM_NULL);
            break;
        ...
        case 6:
            ...
            if (my_world_rank == 0) 
            {
                err = MPI_Allgather(sendbuf, size, MPI_INT, recvbuf, size, MPI_INT, MPI_COMM_SELF);
            } 
            else 
            {
                err = MPI_Allgather(sendbuf, size, MPI_INT, recvbuf, size, MPI_INT, MPI_COMM_WORLD);
    ...
    if (err == MPI_SUCCESS) {
        printf("Process %d found no error in allgather test %d \n", my_world_rank, test_num);
    } else {      
