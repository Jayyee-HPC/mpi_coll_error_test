...
int Error_Test_allreduce(int argc, char **argv)
{
    ...
    switch(test_num)
    {
        case 0:
            ...
            err = MPI_Allreduce(sendbuf, recvbuf, size, MPI_INT, MPI_MAX, MPI_COMM_WORLD);
        ...
        case 3:
            ...
            if (my_world_rank == 0) 
            {
                err = MPI_Allreduce(sendbuf, recvbuf, size, MPI_UNSIGNED, MPI_MAX, MPI_COMM_WORLD);
            } 
            else 
            {
                err = MPI_Allreduce(sendbuf, recvbuf, size, MPI_INT, MPI_MAX, MPI_COMM_WORLD);
        ...
        case 6:
            ...
            if (my_world_rank == 0) 
            {
                err = MPI_Allreduce(sendbuf, recvbuf, size, MPI_INT, MPI_MAX, MPI_COMM_SELF);
            } 
            else 
            {
                err = MPI_Allreduce(sendbuf, recvbuf, size, MPI_INT, MPI_MAX, MPI_COMM_WORLD);
        ...
        case 7:
            ...
            {
                err = MPI_Allreduce(sendbuf, recvbuf, size, MPI_INT, MPI_MIN, MPI_COMM_WORLD);
            }
