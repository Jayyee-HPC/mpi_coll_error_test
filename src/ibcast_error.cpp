...
int Error_Test_ibcast(int argc, char **argv)
{
    ...
    switch(test_num)
    {
        case 0:
            ...
            err = MPI_Ibcast(sendbuf, size, MPI_INT, root, MPI_COMM_WORLD, &request);
        ...
        case 3:
            ...
            if (my_world_rank == 0) 
            {
                err = MPI_Ibcast(sendbuf, size, MPI_UNSIGNED, root, MPI_COMM_WORLD, &request);
            } 
            else 
            {
                err = MPI_Ibcast(sendbuf, size, MPI_INT, root, MPI_COMM_WORLD, &request);
        ...
        case 5:
            ...
            err = MPI_Ibcast(sendbuf, size, MPI_INT, root, MPI_COMM_NULL, &request);
            MPI_Wait(&request, &status);
            break;
        ...
        case 6:
            ...
            if (my_world_rank == 0) 
            {
                err = MPI_Ibcast(sendbuf, size, MPI_INT, root, MPI_COMM_SELF, &request);
            } 
            else 
            {
                err = MPI_Ibcast(sendbuf, size, MPI_INT, root, MPI_COMM_WORLD, &request);
    ...
    if (err == MPI_SUCCESS) {
        printf("Process %d found no error in ibcast test %d \n", my_world_rank, test_num);
    } else {      
