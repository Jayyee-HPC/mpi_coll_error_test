...
int Error_Test_allgatherv(int argc, char **argv)
{
    ...
    switch(test_num)
    {
        ...
        case 3:
            ...
            if (my_world_rank == 0) 
            {
                err = MPI_Allgatherv(sendbuf, size, MPI_UNSIGNED, recvbuf, recvcounts, displs, MPI_UNSIGNED, MPI_COMM_WORLD);
            } 
            else 
            {
                err = MPI_Allgatherv(sendbuf, size, MPI_INT, recvbuf, recvcounts, displs, MPI_INT, MPI_COMM_WORLD);
        ...
        case 5:
            ...
            if (my_world_rank == 0) 
            {
                err = MPI_Allgatherv(sendbuf, size, MPI_INT, recvbuf, recvcounts, displs, MPI_INT, MPI_COMM_SELF);
            } 
            else 
            {
                err = MPI_Allgatherv(sendbuf, size, MPI_INT, recvbuf, recvcounts, displs, MPI_INT, MPI_COMM_WORLD);
