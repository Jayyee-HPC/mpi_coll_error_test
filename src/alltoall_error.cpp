...
int Error_Test_alltoall(int argc, char **argv)
{
    ...
    switch(test_num)
    {
        case 0:
            ...
            err = MPI_Alltoall(sendbuf, size, MPI_INT, recvbuf, size, MPI_INT, MPI_COMM_WORLD);
        ...
        case 3:
            ...
            if (my_world_rank == 0) 
            {
                err = MPI_Alltoall(sendbuf, size, MPI_UNSIGNED, recvbuf, size, MPI_UNSIGNED, MPI_COMM_WORLD);
            } 
            else 
            {
                err = MPI_Alltoall(sendbuf, size, MPI_INT, recvbuf, size, MPI_INT, MPI_COMM_WORLD);
        ...
        case 5:
            ...
            if (my_world_rank == 0) 
            {
                err = MPI_Alltoall(sendbuf, size, MPI_INT, recvbuf, size, MPI_INT, MPI_COMM_SELF);
            } 
            else 
            {
                err = MPI_Alltoall(sendbuf, size, MPI_INT, recvbuf, size, MPI_INT, MPI_COMM_WORLD);
    ...
    if (err == MPI_SUCCESS) {
        printf("Process %d found no error in alltoall test %d \n", my_world_rank, test_num);
    } else {      
